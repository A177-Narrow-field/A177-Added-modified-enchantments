...
public class SlowHeavyWeaknessEnchantment extends Enchantment {
    
    private static AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.ENCHANTMENTS.get("slow_heavy_weakness");
    }
    
    public SlowHeavyWeaknessEnchantment() {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        if (event.getSource().getEntity() instanceof Player player) {
            ...
            int level = weapon.getEnchantmentLevel(ModEnchantments.SLOW_HEAVY_WEAKNESS.get());
            ...
            if (level > 0 && event.getEntity() instanceof LivingEntity) {
                ...
                if (target.hasEffect(MobEffects.MOVEMENT_SLOWDOWN)) {
                    float newDamage = event.getAmount() * 1.3f;
