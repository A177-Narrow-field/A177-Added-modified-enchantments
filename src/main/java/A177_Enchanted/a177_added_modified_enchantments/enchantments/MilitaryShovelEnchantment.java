...
public class MilitaryShovelEnchantment extends Enchantment {
    // 挖掘距离增加的UUID
    public static final UUID BLOCK_REACH_MODIFIER_UUID = UUID.fromString("C1D2E3F4-A5B6-C7D8-E9F0-1234567890AB");
    
    // 挖掘速度增加的UUID
    public static final UUID DIG_SPEED_MODIFIER_UUID = UUID.fromString("D2E3F4A5-B6C7-D8E9-F012-34567890ABCD");
    
    private static AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.ENCHANTMENTS.get("military_shovel");
    }

    public MilitaryShovelEnchantment() {
    ...
    protected boolean checkCompatibility(Enchantment other) {
        return super.checkCompatibility(other) && other != ModEnchantments.UNIVERSAL_SHOVEL.get();
    }
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        if (!tool.isEmpty() && tool.isEnchanted() && tool.getEnchantmentLevel(ModEnchantments.MILITARY_SHOVEL.get()) > 0) {
            int level = tool.getEnchantmentLevel(ModEnchantments.MILITARY_SHOVEL.get());
            // 每级增加400%挖掘速度
            event.setNewSpeed(event.getOriginalSpeed() * (1.0f + level * 4.0f));
        }
    }
        ...
        if (!tool.isEmpty() && tool.isEnchanted() && tool.getEnchantmentLevel(ModEnchantments.MILITARY_SHOVEL.get()) > 0) {
            ...
            if (player.getAttribute(ForgeMod.BLOCK_REACH.get()) != null) {
                AttributeModifier blockReachModifier = new AttributeModifier(
                        ...
                        "Military shovel block reach bonus",
                        level * 1.0,
