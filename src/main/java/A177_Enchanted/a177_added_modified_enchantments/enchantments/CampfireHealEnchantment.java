...
public class CampfireHealEnchantment extends Enchantment {
    ...
    public CampfireHealEnchantment() {
    ...
    public AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.CAMPFIRE_HEAL;
    }
    ...
    protected boolean checkCompatibility(Enchantment other) {
        return super.checkCompatibility(other) && other != ModEnchantments.HABITAT.get();
    }
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        // 检查胸甲是否有篝愈附魔
        int level = chestItem.getEnchantmentLevel(ModEnchantments.CAMPFIRE_HEAL.get());
        if (level <= 0) {
            return;
        }
        
        // 检查5格范围内是否有篝火
        boolean hasCampfireNearby = false;
        BlockPos playerPos = player.blockPosition();
        
        // 检查玩家周围5格范围内的方块
        for (int x = -5; x <= 5; x++) {
            for (int y = -5; y <= 5; y++) { // 减小Y轴范围以提高性能
                for (int z = -5; z <= 5; z++) {
