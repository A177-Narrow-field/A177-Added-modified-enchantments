...
public class StinkyFeetEnchantment extends Enchantment {
    // 缓存玩家当前的附魔等级，避免重复计算
    private static final WeakHashMap<Player, Integer> PLAYER_STINKY_FEET_CACHE = new WeakHashMap<>();
    ...
    private static AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.ENCHANTMENTS.get("stinky_feet");
    }

    public StinkyFeetEnchantment() {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        // 获取缓存的附魔等级
        int cachedLevel = PLAYER_STINKY_FEET_CACHE.getOrDefault(player, -1);
        int currentLevel = 0;
        
        // 检查鞋子是否有脚臭附魔
        ItemStack boots = player.getItemBySlot(EquipmentSlot.FEET);
        if (!boots.isEmpty() && boots.isEnchanted() && boots.getEnchantmentLevel(ModEnchantments.STINKY_FEET.get()) > 0) {
            currentLevel = boots.getEnchantmentLevel(ModEnchantments.STINKY_FEET.get());
        }
        
        // 如果等级发生变化，更新缓存
        if (cachedLevel != currentLevel) {
            PLAYER_STINKY_FEET_CACHE.put(player, currentLevel);
        }
        
        // 如果有附魔等级，处理中毒效果逻辑
        if (currentLevel > 0) {
            applyPoisonEffectToNearbyEntities(player, currentLevel);
        }
    }
    ...
    public static void onLivingEquipmentChange(LivingEquipmentChangeEvent event) {
        ...
        // 当鞋子装备变更时，立即清除缓存
        PLAYER_STINKY_FEET_CACHE.remove(player);
     ...
     * @param level 附魔等级
     */
    private static void applyPoisonEffectToNearbyEntities(Player player, int level) {
        // 计算中毒效果持续时间（每级增加3秒，基础3秒）
        int duration = (3 + level * 3) * 20; // 转换为tick单位
