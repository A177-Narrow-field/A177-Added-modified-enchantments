...
public class RepeatingBoltEnchantment extends Enchantment {
    
    private static AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.ENCHANTMENTS.get("repeating_bolt");
    }
    
    public RepeatingBoltEnchantment() {
    ...
    public static void onEntityJoinWorld(EntityJoinLevelEvent event) {
        ...
        if (event.getEntity() instanceof AbstractArrow arrow && !arrow.level().isClientSide) {
            ...
            if (arrow.getOwner() instanceof Player player) {
                ...
                // 检查武器是否有重程弩附魔
                int level = weapon.getEnchantmentLevel(ModEnchantments.REPEATING_BOLT.get());
                if (level > 0) {
                    // 每级提升2点伤害
                    double damageBonus = level * 2.0;
