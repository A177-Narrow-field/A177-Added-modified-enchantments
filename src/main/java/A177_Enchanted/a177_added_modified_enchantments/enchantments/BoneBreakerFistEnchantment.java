...
public class BoneBreakerFistEnchantment extends Enchantment {
    
    private static AllEnchantmentsConfig.EnchantConfig getConfig() {
        return AllEnchantmentsConfig.ENCHANTMENTS.get("bone_breaker_fist");
    }

    public BoneBreakerFistEnchantment() {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (event.getEntity() instanceof LivingEntity && event.getSource().getEntity() instanceof Player player) {
            ...
            int enchantmentLevel = EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.BONE_BREAKER_FIST.get(), chestplate);

            // 只有在玩家空手且胸甲有附魔时才应用效果
            if (isFistFighting && enchantmentLevel > 0) {
                // 每级增加5点真实伤害
                float trueDamage = enchantmentLevel * 5.0f;
